...
makeCacheMatrix <- function(x = matrix()) {
  ...
  m     <- x     # cache the passed in matrix
  m_inv <- NULL  # init the inverse to NULL
  ...
  setinv <- function(inv) {
    ...
    if (!isTRUE(all.equal((inv %*% m),diag(nrow(m)))))
    {
      m_inv <<- NULL
...
cacheSolve <- function(x, ...) {
  # Get the inverse of the cached matrix. If the inverse
  # was previously computed it is used, otherwise the 
